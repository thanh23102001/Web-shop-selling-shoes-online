...
public class EmployeeUpdateProduct extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        //img
        ArrayList<Images> imgs = (new ImageDAO()).getImagesByProductId(pid);
        JsonArray imgsJson = gson.toJsonTree(imgs).getAsJsonArray();
        request.setAttribute("imgsJ", imgsJson);
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (!"".equals(photos.get(0))) {
            iDao.deleteImagesByProductId(pid + "");
            for (String p : photos) {
                if (photos != null) {
                    iDao.addImage(pid + "", p);
                }
